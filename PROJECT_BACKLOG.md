...
## 서버 배포 시 재개: YouTube 자동 검색

- [ ] 서버 배포 시 새 YouTube API 키를 발급하고 Docker 컨테이너 실행 환경에 `YOUTUBE_API_KEY`로 주입한다.

2026-09-13 결정: 그때까지 키를 설정하지 않고 자동 검색을 비활성화한다. 현재 설정의 빈 키 기본값과 서비스의 503 응답 처리를 그대로 사용하며, Google 검색 API를 호출하지 않는다. PDF 열기와 YouTube 링크 직접 입력은 계속 사용한다.

재개 시 새 키의 API 제한을 YouTube Data API v3로 지정하고, 키를 소스나 Docker 이미지에 포함하지 않는다. 완료 기준은 배포 서버에서 자동 검색으로 영상 ID를 받아 실제 영상 연결까지 확인하는 것이다. 상세 설정 방법은 [README.md](README.md#3-youtube-자동-검색용-서버-api-키)를 참고한다.
